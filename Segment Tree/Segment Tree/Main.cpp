...
// 区间修改
void Update(int L, int R, int C, int l, int r, int p) { // 将[L,R] += C, l,r为当前区间,p为当前树节点
	if (L <= l && r <= R) { // 找到目标区间
		// 更新当前值
		tree[p] += (r - l + 1) * C;
		// 更新懒惰标记
...
int main() {
	...
	Build(1, 13, 1);
	cout << Query(1, 13, 1, 13, 1) << endl; // 31

	Update(6, 7, 1, 13, 1);  // A[6] += 7;
	cout << Query(1, 13, 1, 13, 1) << endl; // 38

	Update(6, 7, 1, 13, 1);  // A[6] += 7;
	cout << Query(2, 12, 1, 13, 1) << endl; // 43

	Update(2, 12, 1, 1, 13, 1); // [2, 12] += 1;
	cout << Query(2, 12, 1, 13, 1) << endl; // 54
	return 0;
}